...
const pathMap: { [key: string]: string } = {
  ...
  transitionAppUserToPathwayStage: 'appusers/{{appUserId}}/pathways/{{appUserPathwayId}}/transition/',
...
class PathwaysAdminClient implements IPathwaysAdminClient {
  ...
  deletePathwayStage = (pathwayId: number, stageId: number) => {
    return this.deleteRequest(
      ...
      { pathwayId: `${pathwayId}`, stageId: `${stageId}` }
    );
  };
  ...
  patchPathway = (pathwayId: number, pathwayData: IPathwayData) => {
    ...
    return this.patchRequest('patchPathway', patchData, 'Unable to update Pathway', { pathwayId: `${pathwayId}` });
  };
  ...
  patchRule = async (ruleId: number, ruleData: IRuleData) => {
    ...
    return this.patchRequest('patchRule', patchData, 'Unable to update Rule', { ruleId: `${ruleId}`});
  };
